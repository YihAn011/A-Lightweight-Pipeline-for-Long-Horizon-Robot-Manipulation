...
def mouse_callback(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        cv2.imshow("Annotation Tool - VAL", img_copy)
...
def annotate_val_set():
    global current_image, current_image_path, boxes
    
    print("="*70)
    print("Annotate Validation Set")
    print("="*70)
    ...
    cv2.namedWindow("Annotation Tool - VAL", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Annotation Tool - VAL", 1280, 960)
    cv2.setMouseCallback("Annotation Tool - VAL", mouse_callback)
    ...
    for idx, img_path in enumerate(val_images):
        ...
        print(f"\n[{idx+1}/{len(val_images)}] {img_path.name}")
        cv2.imshow("Annotation Tool - VAL", current_image)
        ...
        while True:
            ...
            if key == 13:
                ...
                print(f"\nCompleted!")
                print(f"   Annotated: {annotated_count}/{len(val_images)}")
